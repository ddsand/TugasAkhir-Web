...
class DB {
	...
	//rest 
	public function post_product($obj, $pk, $cat, $column_names, $table_name){
		$resp 	 = array();
		$keys 	 = array_keys($obj);
		$columns = '';
		$values  = '';
		...
		$qcat = "SELECT id FROM category WHERE name='".$cat."'";
		$restt = $this->mysqli->query($qcat);
		if($restt){
			$catrow = $restt->fetch_assoc();
			$idcat = $catrow['id'];
		}
		$query = "INSERT INTO ".$table_name."(".trim($columns,',').") VALUES(".trim($values,',').")";
		if(!empty($obj)){
			if ($this->mysqli->query($query)) {
				...
				$selcat = "SELECT * FROM product_category WHERE product_id='".$last_id."'";
				$s = $this->mysqli->query($selcat);
				if($s->num_rows>0){
						$upcat = "UPDATE product_category SET category_id='".$idcat."' WHERE product_id='".$last_id."'";
						$succupdate = $this->mysqli->query($upcat);
				}else{
					$get_cat = "INSERT INTO product_category (product_id,category_id) VALUES ('".$last_id."','".$idcat."')";
					$catquery = $this->mysqli->query($get_cat);
				}
