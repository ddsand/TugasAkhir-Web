...
class API {
	...
	//USER
	private function registasiUMKM(){
		$this->client->registUMKM();//baru
	}
	//UMKM
	private function insProduct(){
		$this->client->inProduct();
	}
	private function delsProduct(){
		$this->client->delProduct();
	}
	private function productUser(){
		$this->client->detailsProductUser();
	}
	public function allproductuser(){
		$this->client->allProductUser();
	}
	private function processLog(){
		$this->client->findUserApi();
	}
	private function processRegist(){
		$this->client->registUser();
	}
	private function allOrder(){
		$this->client->OrderCek();
	}
	private function detailOrder(){
		$this->client->detailOrder();
	}

	//ADMIN
	private function processAllUMKM(){
		$this->client->findAllUMKM();
	}
	private function processDetailUMKM(){
		$this->client->detailInfoUMKM();
	}
	private function processUnverified(){
		$this->client->findUnverfikasiUMKM();
	}
	private function processUMKM(){
		$this->client->verificationUMKM();
	}
	private function processDelUMKM(){
		$this->client->DelStatusUMKM();
	}
	private function processUpPassword(){
		$this->client->upPassAdmin();
	}
	private function processListOrder(){
		$this->client->cekOrder();
	}
	private function processUpOrder(){
		$this->client->upOrder();
	}
	private function processSet(){
		$this->client->setOrder();
	}

	//prob
	private function getWarn(){
		//$this->client->sendAllWarn();
			$this->fcm->sendNotif();
	}
	//Transaksi
	private function ManualPay(){
		$this->client->ManualPay();
	}
	private function NotifVA(){
		$this->client->nomorVA();
	}
    ...
    // private function getLoginApi(){
    // 	$this->client->findUserApi();
    // }

    private function LogoutApi(){
    	$this->client->LogOutAPI();
    }
    private function processSaldo(){
    	$this->client->SaldoUser();
    }

    private function requestEzpy(){
    	$this->client->requestEzpy();
    }

  	private function rqVirtual(){
  		$this->client->reqNomorVA();
  	}
	/*
