...
class Product extends REST{
	...
	public function findOneDetailUser($id,$iduser){
		$query = "SELECT * FROM product p WHERE p.id=$id AND p.iduser=$iduser";
		return $this->db->get_list($query);
	}
	public function findProductUser($iduser){
		$query = "SELECT * FROM product p WHERE p.iduser=$iduser";
		return $this->db->get_list($query);
	}
