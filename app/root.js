angular.module('App').controller('RootCtrl', function ($rootScope, $scope, $mdSidenav, $mdToast, $mdDialog, $cookies, request, focus) {
    ...
    self.uid_status = root.base_url+ '_session_status';
    ...
    root.saveCookies = function (id, name, email, password,status) {
        // saving session
        var now = new Date();
        ...
        $cookies.put(self.uid_status, status);
    ...
    root.isCookieExist = function () {
        ...
        var status = $cookies.get(self.uid_status);
        if (uid == null || name == null || status || email == null || password == null) {
            return false;
        }
        return true;
    };
    ...
    root.getSessionStatus = function(){
        return $cookies.get(self.uid_status);
    }
